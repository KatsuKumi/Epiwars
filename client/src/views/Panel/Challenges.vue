<template>
    <div class="fill-height">
        <PanelNavigation :selected-item="1"></PanelNavigation>
        <v-container>
            <v-row>
                <v-col cols="12">
                    <h1 class="display-1 text-center">Challenges</h1>
                </v-col>
            </v-row>
            <v-row>
                <v-col cols="12">
                    <v-card class="pa-2 rounded-lg mx-auto my-4" min-height="700" elevation="2"
                            color="#222327">
                        <v-card-title>
                            <span class="headline">Challenges</span>
                        </v-card-title>
                            <v-row align="center" justify="center" class="px-3">
                                <v-col cols="6">
                                    <v-text-field
                                        v-model="search"
                                        class="pt-0"
                                        append-icon="mdi-magnify"
                                        label="Search"
                                        single-line
                                        hide-details
                                    ></v-text-field>
                                </v-col>
                                <v-col cols="6" class="text-right">
                                    <v-dialog
                                        v-model="createDialog"
                                        persistent
                                        max-width="900px"
                                    >
                                        <template v-slot:activator="{ on, attrs }">
                                            <v-btn
                                                class="mx-2"
                                                dark
                                                outlined
                                                color="indigo"
                                                v-bind="attrs"
                                                v-on="on"
                                            >
                                                <v-icon dark>
                                                    mdi-plus
                                                </v-icon>
                                                Create
                                            </v-btn>
                                        </template>

                                        <ChallengeForm :opened="createDialog" v-on:close="createDialog = false" v-on:save="createChallenge" :loading="createLoading"></ChallengeForm>
                                    </v-dialog>
                                </v-col>
                            </v-row>
                            <v-row>
                                <v-col cols="12">
                                    <TableChallenge :kataDialog="kataDialog"
                                                    :challenges="challenges"
                                                    :search="search"
                                                    v-on:snackbar="showSnackbar"
                                                    v-on:update="updateChallenges"
                                                    v-on:close="kataDialog = false">
                                    </TableChallenge>
                                </v-col>
                            </v-row>
                    </v-card>
                </v-col>
            </v-row>
        </v-container>
        <v-snackbar
          v-model="snackbar.visible"
          :timeout="snackbar.timeout"
        >
          {{ snackbar.text }}

          <template v-slot:action="{ attrs }">
            <v-btn
              color="blue"
              text
              v-bind="attrs"
              @click="snackbar.visible = false"
            >
              Close
            </v-btn>
          </template>
        </v-snackbar>
    </div>
</template>

<script>
import PanelNavigation from "@/components/PanelNavigation";
import {DateTime} from "luxon";
import TableChallenge from "@/components/Panel/Challenge/TableChallenge";
import axios from "axios";
import ChallengeForm from "@/components/Panel/Challenge/ChallengeForm";

export default {
    name: 'Panel',
    components: {ChallengeForm, PanelNavigation, TableChallenge},
    data() {
        return {
            createDialog: false,
            createLoading: false,
            kataDialog: false,
            search: '',
            snackbar: {
                text: '',
                timeout: 3000,
                visible: false,
            },
            challenges: [],
        }
    },
    methods: {
        save(date) {
            this.$refs.menu.save(date)
        },
        showSnackbar(text) {
            this.snackbar.text = text;
            this.snackbar.visible = true;
        },
        createChallenge(item) {
            if (item.name === '' || item.description === '' || item.startDate === null || item.startTime === null) {
                return;
            }
            this.createLoading = true;
            let hour = item.startTime.split(":")[0];
            let minute = item.startTime.split(":")[1];
            let dateTime = DateTime.fromISO(item.startDate).set({hour, minute}).toISO();
            axios.post('/api/challenges/', {
                name: item.name,
                description: item.description,
                startDate: dateTime,
            }).then(response => {
                this.updateChallenges(() => {
                    this.showSnackbar('Challenge created');
                    this.createDialog = false;
                    this.createLoading = false;
                });
            }).catch(error => {
                console.log(error);
                this.createLoading = false;
            });
        },
        updateChallenges(callback=null) {
            axios.get('/api/challenges/').then(response => {
                this.challenges = response.data;
                if (callback !== null) {
                    callback();
                }
            }).catch(error => {
                console.log(error);
            });
        }
    },
    mounted() {
        this.$store.commit("hideFooter");
        this.updateChallenges();
    },
    watch: {
        menu(val) {
            val && setTimeout(() => (this.activePicker = 'YEAR'))
        },
    },
}
</script>