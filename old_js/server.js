...
passport.use(new GithubStrategy({
	...
	app.get("db").read.user_by_github_id([profile.id]).then((user) => {
		console.log('Suscl');
		if (user[0]) {
			console.log(user[0])
			done(null, user[0]);
			...
			app.get("db").create.new_user_from_github([profile.id, profile.displayName, profile._json.email, profile.username, profile._json.avatar_url],

				(err) => {
					if (err) {
						console.log(err);
					}	else {
						app.get("db").read.user_by_github_id([profile.id], (err, newUser) => {
							if(err) {
								console.log(err);
							} else {
								done(null, newUser[0])
...
massive({
	...
	database: 'epiwars',
	user: 'epiwars',
	password: 'd}6gB*jNYnKQX=CJ',
	ssl: false,
	poolSize: 10
  }).then(massiveInstance => {
    app.set('db', massiveInstance);
	monitor.attach(massiveInstance.driverConfig);
